...
export function calculateTimeElapsed(timestamp: number, multiplier: number): TimeElapsed {
  const now = dayjs(); // Current time
  const target = dayjs(timestamp);


  const diff = now.diff(target) * multiplier; // Difference in milliseconds
...
export function calculateTimeLeft(timestamp: number): TimeLeft {
  const now = dayjs(); // Current time
  const target = dayjs(timestamp);
